...
| `productionQueue` | `List<ProductionData>` | 생산 대기열 (최대 5개) |
...
| `Enqueue(unitID)` | 지정한 유닛 ID를 생산 대기열에 추가한다. 대기열이 5개 이상이면 무시된다. (자원 소모는 호출측인 `RTSUnitController.TryProduceUnit`에서 먼저 처리된 뒤 호출됨) |
...
| `Cancel(index)` | 대기열의 특정 인덱스 항목을 취소(제거). UI의 대기열 슬롯 클릭 시 호출 |
...
- **BuildingController**: `SpawnUnit`/`GetProductionQueue`/`GetProductionProgress`/`CancelProduction`을 이 컴포넌트에 위임

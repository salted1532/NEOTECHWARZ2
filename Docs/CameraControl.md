...
| `zoomSpeed`, `minZoom`, `maxZoom` | `float` | 줌 속도, 카메라 높이(Y) 하한/상한 (기준 지형고도 0 기준. 너무 가깝게/멀리 못 가게) |
| `groundLayer` | `LayerMask` | 화면 중앙 지형 판정용 레이어(`Ground`). `SampleTerrainTier()`가 이 레이어로 레이캐스트해서 지형 단(tier)을 알아냄 |
| `tierZoomStep` | `float` | 지형 단(`Layer1`/`Layer2` 태그) 하나당 `minZoom`/`maxZoom`과 현재 줌 높이가 같이 움직이는 양 (기본 5) |
...
| `currentTerrainTier` | `int` | 화면 중앙이 보고 있는 지형의 높이 단. 0=지상, 1=언덕(`Layer1`), 2=언덕 위 언덕(`Layer2`) |
...
| `Update()` | 매 프레임 `HandleMovement()` + `HandleTerrainTier()` + `HandleZoom()` + `HandleRotate()` 호출 |
...
| `HandleZoom()` (private) | 이 카메라는 Perspective라 `orthographicSize`가 효과 없으므로, 마우스 휠 입력으로 카메라가 바라보는 방향(forward)을 따라 위치를 직접 앞뒤로 옮겨서 확대/축소를 구현. 높이(Y) 기준 `minZoom + tierOffset`~`maxZoom + tierOffset` 범위로 제한(`tierOffset = currentTerrainTier * tierZoomStep`) — 언덕 위를 보고 있으면 줌 범위 전체가 단(tier)만큼 같이 올라가서 평지에서와 동일한 거리감을 유지 |
| `HandleTerrainTier()` (private) | 매 프레임 `SampleTerrainTier()`로 화면 중앙 지형의 단을 다시 재서 `currentTerrainTier`와 비교. 바뀌었으면 그 차이(`(newTier - currentTerrainTier) * tierZoomStep`)만큼 `targetPosition.y`를 즉시 밀어 올리거나 내려서, 스크롤 없이 카메라가 지형을 넘나들 때도(패닝만으로도) 줌 높이가 자동으로 따라감 |
| `SampleTerrainTier()` (private) | 화면 정중앙 레이를 `groundLayer`로 레이캐스트하고, 맞은 콜라이더의 트랜스폼에서부터 부모 방향으로 올라가며 `Layer2`/`Layer1` 태그를 찾아 지형 단(0/1/2)을 판정. 실제 지형 메시/콜라이더가 태그 붙은 오브젝트의 자식(타일 프리팹)에 있는 구조를 고려한 것 |

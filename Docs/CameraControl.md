...
| `groundLayer` | `LayerMask` | 화면 중앙 지형 높이 샘플링용 레이어(`Ground`). 언덕 위/아래에서 `minZoom`/`maxZoom` 판정에 지형 높이를 반영하는 데 사용 |
...
| `HandleZoom()` (private) | 이 카메라는 Perspective라 `orthographicSize`가 효과 없으므로, 마우스 휠 입력으로 카메라가 바라보는 방향(forward)을 따라 위치를 직접 앞뒤로 옮겨서 확대/축소를 구현. 높이(Y) 기준 `minZoom + groundHeight`~`maxZoom + groundHeight` 범위로 제한(`groundHeight`는 `GetGroundHeightAtScreenCenter()`가 매 스크롤마다 다시 계산) — 언덕 위를 보고 있으면 줌 범위 전체가 그 높이만큼 같이 올라가서 평지에서와 동일한 거리감을 유지 |
...
| `GetGroundHeightAtScreenCenter()` (private) | 화면 정중앙 레이를 `groundLayer`로 레이캐스트해 실제 지형(언덕 등) 높이를 구한다(`HandleZoom()` 전용, 위 메소드와 달리 실제 콜라이더 기반). `groundLayer` 미설정이거나 아무것도 맞지 않으면 0(평지 기준) 반환 |

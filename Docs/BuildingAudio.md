...
| `HandleDamaged(amount, attackerPosition, attackType, isEnemyAttacker)` (private) | `OnDamaged` 콜백 — 화면 밖에서 공격받았고 `isEnemyAttacker`가 true일 때만 `PlayBuildingUnderAttackWarning()` 호출(doc/0292) |

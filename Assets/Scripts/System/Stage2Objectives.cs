...
public class Stage2Objectives : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (artifactDelivered)
            StageManager.Instance?.ReportVictory(); // 주목표 성공 사운드는 이제 StageManager.ReportVictory() 안에서 공통으로 재생됨(doc/0643)

        // 서브목표(연구 데이터) 반납 완료 시에도 재생한다(doc/0465). delivered 플래그는 한 번 켜지면
        // 계속 true라 Update()가 매 프레임 여기로 들어오므로, 성공 SFX는 최초 1회만 울리도록 별도
        // 플래그로 막는다(doc/0464).

...
// 아군 OC(플레이어에게 적대적이지 않은 진영) 유닛 컨트롤러. EnemyUnitController를 상속하지 않고 이동/전투
// AI 로직을 통째로 복제한 완전 독립 클래스 - 아군 AI를 EnemyUnitController와 별개로 자유롭게 발전시킬 수
// 있게 하기 위함(doc/0452, 상속으로 묶어두면 한쪽을 고칠 때 다른 쪽에 영향이 생길 수 있어서 분리함).
// EnemyUnitController와 다른 부분은 피아식별 방향이 반대인 두 곳뿐(Attack()의 isEnemyAttacker,
// HandleAttacked()의 반격 조건) - 나머지는 기능적으로 100% 동일하게 시작한다.
public class AllyController : MonoBehaviour, IDestructible, IAttackRangeUnit
{
    [SerializeField]
    ...
    // ApplyUnitData()로 덮어쓴다 (UnitController.unitID와 동일한 패턴, doc/0232). 아군 OC도 같은 로스터/SO를
    // 그대로 재사용하므로(doc/0447/0448) 조회 대상 SO/필드 이름은 EnemyUnitController와 동일하게 둔다.
    [SerializeField]
    ...
    private EnemyAttackRange attackRange; // 사거리 내 대상 감지용 (자식 컴포넌트, 실제로는 AllyAttackRange - doc/0448)
    ...
    // EnemyUnitController와 반대 조건: 플레이어가 강제공격(doc/0450, isEnemyAttacker=false)했을 땐 반격하러
    // 쫓아가지 않고, 실제 적대 세력(외계종족/OC, isEnemyAttacker=true)에게 공격받았을 때만 반격한다(doc/0452).
    ...
    // 선택 해제(doc/0360, 선택 중인 대상이 안개 속으로 들어가면 선택을 풀어줌)가 같은 안개 조회 결과를
    ...
    private void UpdateFogVisibility()
    {
        ...
        if (!revealed)
            rtsController?.ClearSelectedAllyIfMatches(this);
    }
    ...
    public void AttackMoveTo(Vector3 destination)
    {
        ...
        currentState = AllyState.Idle; // Idle이어야 AllyAttackRange가 사거리 내 상대를 자동으로 교전한다

        MoveAgentTo(destination);
    }

    // Idle 상태에서 사거리 밖의 감지된 상대에게 다가갈 때 AllyAttackRange가 호출한다.
    ...
    public void Attack(Vector3 end, GameObject target)
    {
        ...
        if (target.TryGetComponent<HealthManager>(out var targetHealth))
        {
            ...
            // 공격자는 아군(플레이어에게 적대적이지 않음)이므로 isEnemyAttacker=false - EnemyUnitController와
            // 반대(doc/0452) - 그래야 맞은 적대 세력이 "같은 진영이 때렸다"고 착각해 반격을 무시하지 않는다.
            if (attackDelivery == AttackDeliveryType.Projectile && projectileAttack != null)
                projectileAttack.Fire(target.transform, targetHealth, finalDamage, attackType, isEnemyAttacker: false);
            else
                targetHealth.GetDamage(finalDamage, transform.position, attackType, isEnemyAttacker: false);
    ...
    // 공격 명령(우클릭/A 모드)을 받았을 때 "어느 대상이 대상인지" 피드백으로 마커를 짧게 깜빡인다.
    ...
    private IEnumerator FlashMarkerRoutine()
    {
        ...
        bool isSelected = rtsController != null && rtsController.selectedAllyList.Contains(this);
    ...
    // ======================
    // 상태 확인용 (AllyAttackRange용)
    // ======================
    public bool IsIdle() => currentState == AllyState.Idle;
    public bool IsMove() => currentState == AllyState.Move;
    // currentState는 Idle/Move만 쓰고 Attack은 쓰지 않는다(doc/0236 - Attack에 멈추면 교전 종료 후
    // 추적 재개가 안 됨). 대신 "지금 실제 사거리 안에 대상이 있어서 공격 중인가"를 AllyAttackRange에
    ...
    // TurretController가 조준 대상을 물어볼 때 쓰는 AllyAttackRange 접근자 (UnitController.GetAttackRange()와 동일한 용도).
    ...
    // 동일한 패턴 (doc/0230의 OC Unit Data SO를 아군 OC도 그대로 재사용, doc/0447/0448).
    ...
    public void Die()
    {
        rtsController?.selectedAllyList.Remove(this);
